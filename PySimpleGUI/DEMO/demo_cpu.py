    ...
    for count, part in enumerate(particians):
        ...
        try:
            ...
            layout += [[sg.Text(mount, size=(5, 1), key=('-NAME-', mount)),
                        sg.ProgressBar(100, 'h', size=(10, 15), key=(
                            '-PROG-', mount), bar_color=(this_color, bar_color[1])),
                        sg.Text(f'{usage.percent}%', size=(6, 1), key=('-%-', mount)), sg.T(stats_info, size=(30, 1), key=('-STATS-', mount))]]
        except:
            pass
    layout += [[sg.Text('Refresh', font='Any 8',
                        key='-REFRESH-', enable_events=True)]]
...
def main(location):
    ...
    sg.set_options(suppress_error_popups=True,
                   suppress_raise_key_errors=False, suppress_key_guessing=True)
    ...
    try:
        ...
        while True:
            ...
            if event == 'Edit Me':
                ...
                ), keep_on_top=True, location=window.current_location())

            # update the window.. if not True then something changed and need to make a new window
