...
layout = [[sg.Text('Window with 2 collapsible sections')],
          [sg.Checkbox('Blank checkbox'), sg.Checkbox(
              'Hide Section 2', enable_events=True, key='-OPEN SEC2-CHECKBOX-')],
          #### Section 1 part ####
          [Collapsible(section1, SEC1_KEY,  'Section 1', collapsed=True)],
          #### Section 2 part ####
          [Collapsible(section2, SEC2_KEY, 'Section 2', arrows=(
              sg.SYMBOL_TITLEBAR_MINIMIZE, sg.SYMBOL_TITLEBAR_MAXIMIZE))],
...
while True:             # Event Loop
    ...
    if event.startswith(SEC1_KEY):
        ...
        window[SEC1_KEY+'-BUTTON-'].update(window[SEC1_KEY].metadata[0]
                                           if window[SEC1_KEY].visible else window[SEC1_KEY].metadata[1])

    if event.startswith(SEC2_KEY) or event == '-OPEN SEC2-CHECKBOX-':
        window[SEC2_KEY].update(visible=not window[SEC2_KEY].visible)
        window[SEC2_KEY+'-BUTTON-'].update(window[SEC2_KEY].metadata[0]
                                           if window[SEC2_KEY].visible else window[SEC2_KEY].metadata[1])
