...
lista_uno = [1, 2, 3, 4, 5]
lista_due = [6, 7, 8, 9, 10]
lista_somma = [0] * len(lista_due)

if len(lista_due) != len(lista_uno):
    print("Somthings Wrong!")
else:
    for i in range(len(lista_due)):
        lista_somma[i] = lista_uno[i] + lista_due[i]
...
for i in range(len(lista_uno)):
    lista_somma.append((lista_uno[i] + lista_due[i]))

print(f'ciclo con append lista completa {lista_somma}')