...
tot_morti_per_mese = {key: value/len(data.keys())
                      for (key, value) in tot_morti_per_mese_acc.items()}
...
data = pd.DataFrame.from_dict(tot_morti_mese, orient="index")
for val in x:
    data_mese = data[data.index.str.contains(val)]
    print(val, data_mese)
